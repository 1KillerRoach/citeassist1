    ...
    <section className="rounded-2xl border-2 border-slate-900 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 font-mono text-lg text-slate-700">{citationLine(item)}</p>
          <div className="mt-3 flex flex-wrap gap-2">
            <Badge>{item.category}</Badge>
            <Badge level={item.level}>{LEVEL_LABEL[item.level]}</Badge>
          </div>
        </div>
        <button type="button" onClick={onClose} className="rounded-xl border bg-white px-4 py-2">Close</button>
      </div>
      ...
      {url ? (
        <a className="mt-4 inline-block rounded-xl bg-slate-900 px-4 py-2 text-white" href={url} target="_blank" rel="noreferrer">
          Open official code
        </a>
      ) : (
